...
class FinalResults(Page):
    ...
    def vars_for_template(player: Player):
        ...
        capped_total_payment_euros = min(raw_total_payment_euros, C.BONUS_CAP_EUROS)
        capped_total_payment_tokens = capped_total_payment_euros * C.TOKENS_PER_EURO

        participant.vars['investment_payment_average'] = investment_tokens
        participant.vars['holt_laury_payment'] = holt_laury_tokens

        # Raw theoretical amount generated by the experiment
        ...
        # Actual capped performance-based bonus to be paid
        participant.vars['bonus_cap_euros'] = C.BONUS_CAP_EUROS
        participant.vars['capped_total_payment_tokens'] = capped_total_payment_tokens
        participant.vars['capped_total_payment_euros'] = capped_total_payment_euros

        # Legacy/final payment variables now correspond to the capped paid bonus
        participant.vars['total_payment_tokens'] = capped_total_payment_tokens
        participant.vars['total_payment_euros'] = capped_total_payment_euros
        ...
        player.payoff = capped_total_payment_tokens
        ...
        return {
            ...
            # Display raw and capped values
            'total_tokens': round(raw_total_payment_tokens, 2),
            'bonus_euros': round(raw_total_payment_euros, 2),
            'raw_bonus_euros': round(raw_total_payment_euros, 2),
            'bonus_cap_euros': round(C.BONUS_CAP_EUROS, 2),
            'capped_bonus_euros': round(capped_total_payment_euros, 2),
        }
...
def custom_export(players):
    yield [
        ...
        'bonus_cap_euros',
        'capped_total_payment_tokens',
        'capped_total_payment_euros',
    ...
    for p in players:
        ...
        bonus_cap_euros = participant.vars.get('bonus_cap_euros', None)
        capped_total_payment_tokens = participant.vars.get('capped_total_payment_tokens', None)
        capped_total_payment_euros = participant.vars.get('capped_total_payment_euros', None)
        ...
        yield [
            ...
            round(bonus_cap_euros, 2) if bonus_cap_euros is not None else None,
            round(capped_total_payment_tokens, 2) if capped_total_payment_tokens is not None else None,
            round(capped_total_payment_euros, 2) if capped_total_payment_euros is not None else None,
